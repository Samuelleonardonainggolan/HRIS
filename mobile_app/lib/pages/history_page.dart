...
class _HistoryPageState extends State<HistoryPage> {
  ...
  static const _filters = ['Semua', 'Hadir', 'Terlambat', 'Izin', 'Cuti', 'Lembur'];
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final existingKeys = <String>{
        for (final r in merged) _key(r.date)
      };
      ...
      for (final p in pengajuan) {
        ...
        final end = DateTime(
            p.endDate.year,   p.endDate.month,   p.endDate.day);

        print('[History] expanding pengajuan ${p.id} "${p.type}" '
        ...
        while (!cur.isAfter(end)) {
          ...
          if (cur.month == _selectedMonth.month &&
            ...
            if (!existingKeys.contains(k)) {
              ...
              print('[History]   ~ skip ${_fmtDate(cur)} (absensi real sudah ada)');
            }
          }
  ...
  void _applyFilter() {
    ...
    setState(() {
      _filtered = (_all.where((r) {
        final mOk = r.date.month == _selectedMonth.month &&
  ...
  int get _cntHadir  => _all.where((r) => r.status == 'On Time').length;
  int get _cntLate   => _all.where((r) => r.status == 'Late').length;
  int get _cntIzin   => _all.where((r) => r.status == 'Izin').length;
  int get _cntCuti   => _all.where((r) => r.status == 'Cuti').length;
  int get _cntLembur => _all.where((r) =>
      r.status == 'Lembur' || r.status == 'Overtime').length;
                    ...
                    child: _profileImage != null
                        ...
                        : Image.network(_avatarUrl(), fit: BoxFit.cover,
                            errorBuilder: (_, __, ___) => const Icon(
                                Icons.person,
                                color: Color(0xFF135BEC), size: 26)),
                  ),
                ),
              ),
            ),
      ...
      child: Column(children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          IconButton(
              ...
              onPressed: () => _changeMonth(-1), padding: EdgeInsets.zero),
          Column(children: [
            Text('Periode Saat Ini',
                style: TextStyle(
                    color: Colors.white.withOpacity(0.8), fontSize: 11)),
            Text(_fmt(_selectedMonth, 'MMMM yyyy'),
                style: const TextStyle(color: Colors.white, fontSize: 18,
                    fontWeight: FontWeight.bold)),
          ]),
          IconButton(
              icon: Icon(Icons.chevron_right,
                  color: isCurrent ? Colors.white30 : Colors.white, size: 22),
        ...
        Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
          _stat('HADIR',     '$_cntHadir',  Colors.white),
          _vDiv(),
  ...
  Widget _buildBody() {
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (_, i) {
          final r = _filtered[i];
          return r.isLeaveRecord
              ? _buildLeaveCard(r)
              : _buildAttendanceCard(r);
        },
      ),
    );
  }
  ...
  Widget _buildAttendanceCard(AttendanceRecord r) {
    ...
    final hasBreak = (r.breakStart?.isNotEmpty ?? false) ||
    ...
    return Container(
      ...
      child: Column(children: [
          ...
          child: Row(
            ...
            Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                ...
                Text(_fmt(r.date, 'EEE, dd MMM yyyy'),
                    style: TextStyle(fontSize: 11, color: Colors.grey.shade500,
                        fontWeight: FontWeight.w500)),
              ]),
              const SizedBox(height: 3),
              Text(r.shiftName.isNotEmpty ? r.shiftName : 'Shift Kerja',
        ...
        Padding(
          ...
          child: Row(children: [
            Expanded(child: _timeChip(Icons.login_rounded, 'JAM MASUK',
                r.clockIn.isEmpty ? '--:--' : '${r.clockIn} WIB',
                const Color(0xFF2ECC71))),
            const SizedBox(width: 10),
            Expanded(child: _timeChip(Icons.logout_rounded, 'JAM KELUAR',
        ...
        if (hasBreak) ...[
          ...
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 16),
            child: Row(children: [
              Expanded(child: _timeChip(Icons.coffee_rounded, 'ISTIRAHAT MULAI',
                  r.breakStart?.isNotEmpty == true
                      ? '${r.breakStart} WIB' : '-',
                  const Color(0xFFF59E0B))),
              const SizedBox(width: 10),
              Expanded(child: _timeChip(Icons.free_breakfast_rounded,
                  'ISTIRAHAT SELESAI',
        ...
        Container(
          ...
          child: Row(
            ...
            Row(children: [
              ...
              const SizedBox(width: 4),
              Text(r.location.isNotEmpty ? r.location : 'Area Hotel',
            ...
            Row(children: [
              ...
              const SizedBox(width: 4),
              Text('${r.workHours.toStringAsFixed(1)} jam',
                  style: const TextStyle(fontSize: 12,
                      color: Color(0xFF135BEC), fontWeight: FontWeight.w600)),
            ...
            Expanded(child: Column(
              ...
              Container(
                ...
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  const SizedBox(width: 4),
                  const Text('PENGAJUAN DISETUJUI',
        ...
        Container(
          ...
          child: Row(children: [
            Icon(Icons.assignment_turned_in_outlined,
                size: 13, color: sc.withOpacity(0.7)),
            const SizedBox(width: 6),
            Text('Tidak hadir · Pengajuan telah disetujui',
  ...
  Color _statusColor(String s) {
    switch (s) {
      case 'On Time':  return const Color(0xFF2ECC71);
      case 'Late':     return const Color(0xFFF59E0B);
      case 'Absent':   return const Color(0xFF94A3B8);
      case 'Overtime': return const Color(0xFF8B5CF6);
      case 'Izin':     return const Color(0xFF135BEC);
