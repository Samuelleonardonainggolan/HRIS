...
class _NewRequestPageState extends State<NewRequestPage> {
  ...
  String get _lemburStr {
    final h = _lemburMins ~/ 60; final m = _lemburMins % 60;
    return m == 0 ? '$h Jam 00 Menit' : '$h Jam $m Menit';
  }

  String get _kompensasiStr {
    final rp = _lemburMins * 10000 ~/ 60;
    if (_compensation == 'Upah') return 'Rp ${NumberFormat('#,###', 'id').format(rp)}';
  ...
  Future<void> _loadTipes() async {
    setState(() { _isLoadingTipes = true; _tipeError = null; });
    try {
  ...
  Future<void> _pickDate({required bool isStart}) async {
    final picked = await showDatePicker(
      ...
      builder: (ctx, child) => Theme(
        data: Theme.of(ctx).copyWith(colorScheme: const ColorScheme.light(primary: Color(0xFF135BEC))),
        child: child!),
    );
    if (picked == null) return;
    setState(() {
      if (isStart) { _startDate = picked; if (_endDate.isBefore(_startDate)) _endDate = _startDate; }
  ...
  Future<void> _pickFile() async {
    final res = await FilePicker.platform.pickFiles(type: FileType.custom, allowedExtensions: ['pdf','png','jpg','jpeg']);
  ...
  void _showSnack(String msg, {bool isError = false}) {
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Text(msg),
      backgroundColor: isError ? const Color(0xFFEF4444) : const Color(0xFF2ECC71),
    ));
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('Pengajuan $_category',
      ...
      body: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          _sectionLabel(_category == 'Lembur' ? 'Alasan / Deskripsi Pekerjaan' : 'Alasan'),
          _reasonField(),
  ...
  Widget _sectionLabel(String t) => Padding(
    ...
    child: Text(t, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Color(0xFF64748B), letterSpacing: 0.3)));

  // ── Selector kategori (Izin / Cuti / Lembur) ──────────────────────────────
  Widget _categorySelector() {
    final categories = ['Izin', 'Cuti', 'Lembur'];
    return Row(children: categories.map((cat) {
        ...
        child: GestureDetector(
          onTap: () {
            setState(() {
              ...
              final matching = _allTipes.where((t) => t.namaKategori == cat).toList();
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(color: sel ? const Color(0xFF135BEC) : Colors.grey.shade200),
  ...
  Widget _tipeSelector() {
    if (_isLoadingTipes) {
      return Container(
        ...
        child: const Row(children: [
          ...
          SizedBox(width: 12),
          Text('Memuat tipe pengajuan...', style: TextStyle(fontSize: 13, color: Color(0xFF94A3B8))),
        ]),
      );
    }
    ...
    if (_tipeError != null) {
      return GestureDetector(
        ...
        child: Container(
          padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14),
          decoration: BoxDecoration(color: const Color(0xFFFFF1F1), borderRadius: BorderRadius.circular(12),
            border: Border.all(color: const Color(0xFFFFCDD2))),
          child: Row(children: [
            const Icon(Icons.refresh_rounded, size: 16, color: Color(0xFFEF4444)),
            const SizedBox(width: 8),
            Text(_tipeError!, style: const TextStyle(fontSize: 13, color: Color(0xFFEF4444))),
            const Spacer(),
            const Text('Coba lagi', style: TextStyle(fontSize: 12, color: Color(0xFFEF4444), fontWeight: FontWeight.w600)),
          ]),
        ),
      );
    }
    ...
    if (options.isEmpty) {
      return Container(
        ...
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12),
          border: Border.all(color: Colors.grey.shade200)),
        child: Text('Tidak ada tipe untuk kategori $_category',
    ...
    return Container(
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<TipePengajuan>(
          ...
          borderRadius: BorderRadius.circular(12),
          hint: Text('Pilih tipe ${_category.toLowerCase()}',
            style: TextStyle(fontSize: 14, color: Colors.grey.shade400)),
          items: options.map((t) => DropdownMenuItem(
            value: t,
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
              Text(t.namaTipe, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: Color(0xFF0F172A))),
              if (t.potongKuota || t.wajibLampiran)
                Row(children: [
                  if (t.potongKuota) _badge('Potong Kuota', const Color(0xFFF59E0B)),
                  if (t.potongKuota && t.wajibLampiran) const SizedBox(width: 4),
                  if (t.wajibLampiran) _badge('Wajib Lampiran', const Color(0xFFEF4444)),
                ]),
            ]),
          )).toList(),
          onChanged: (v) { if (v != null) setState(() => _selectedTipe = v); },
        ),
      ),
    );
  }
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          _dateBox(_fmt.format(_endDate), () => _pickDate(isStart: false)),
        ])),
      ]),
      const SizedBox(height: 12),
      Container(
        ...
        child: Row(children: [
          const Icon(Icons.calendar_month_rounded, color: Color(0xFF135BEC), size: 20),
          const SizedBox(width: 10),
          Text('$_days Hari', style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: Color(0xFF135BEC))),
          if (_selectedTipe?.potongKuota == true) ...[
            const Spacer(),
            const Icon(Icons.info_outline_rounded, size: 14, color: Color(0xFFF59E0B)),
            const SizedBox(width: 4),
            const Text('Memotong kuota cuti', style: TextStyle(fontSize: 11, color: Color(0xFFF59E0B))),
          ]
        ])),
    ]);
  }
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Row(children: [
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          _timeBox(_startTime.format(context), () => _pickTime(isStart: true)),
        ])),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          _sectionLabel('Jam Selesai'),
          _timeBox(_endTime.format(context), () => _pickTime(isStart: false)),
        ])),
      ]),
    ]);
  }
  ...
  Widget _dateBox(String label, VoidCallback onTap) {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(children: [
          const Icon(Icons.calendar_today_rounded, size: 16, color: Color(0xFF135BEC)),
          const SizedBox(width: 8),
          Text(label, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: Color(0xFF0F172A))),
        ])),
    );
  }
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12),
          border: Border.all(color: Colors.grey.shade200)),
        child: Row(children: [
          const Icon(Icons.access_time_rounded, size: 16, color: Color(0xFF135BEC)),
          const SizedBox(width: 8),
          Text(label, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: Color(0xFF0F172A))),
        ])),
    );
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
              color: _file != null
                  ...
                  : (wajib ? const Color(0xFFEF4444).withOpacity(0.4) : Colors.grey.shade200),
              width: _file != null || wajib ? 1.5 : 1),
          ),
          child: _file != null
              ? Row(children: [
                  const Icon(Icons.insert_drive_file_rounded, color: Color(0xFF135BEC), size: 22),
                  const SizedBox(width: 10),
                  Expanded(child: Text(_file!.name,
                    style: const TextStyle(fontSize: 13, color: Color(0xFF135BEC), fontWeight: FontWeight.w500),
                    overflow: TextOverflow.ellipsis)),
                  GestureDetector(onTap: () => setState(() => _file = null),
              ...
              : Column(children: [
                  ...
                  const Text('Surat Keterangan Medis / Dokumen Pendukung',
                  ...
                  Wrap(spacing: 6, children: ['PDF', 'PNG', 'JPG'].map((ext) =>
                    Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 3),
                      decoration: BoxDecoration(color: Colors.grey.shade100, borderRadius: BorderRadius.circular(6)),
                      child: Text(ext, style: TextStyle(fontSize: 10, color: Colors.grey.shade500, fontWeight: FontWeight.w600)))
                  ).toList()),
                ]),
        ),
      ]),
  ...
  Widget _compensationSection() {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      const Padding(
        ...
        child: Text('OPSI KOMPENSASI', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: Color(0xFF64748B), letterSpacing: 0.8))),
      ...['Upah', 'Cuti'].map((opt) {
        ...
        return GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(color: sel ? const Color(0xFF135BEC) : Colors.grey.shade200, width: sel ? 1.5 : 1),
              boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.03), blurRadius: 8)]),
            child: Row(children: [
              Container(height: 20, width: 20,
                decoration: BoxDecoration(shape: BoxShape.circle,
                  border: Border.all(color: sel ? const Color(0xFF135BEC) : Colors.grey.shade400, width: 2),
              ...
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(opt == 'Upah' ? 'Upah Lembur (Uang)' : 'Pertukaran Jam Kerja (Cuti/Off)',
                  style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: Color(0xFF0F172A))),
                Text(opt == 'Upah' ? 'Kompensasi dibayarkan dalam bentuk uang' : 'Ganti waktu lembur dengan pengurangan jam kerja',
  ...
  Widget _lemburSummary() {
    return Container(
      ...
      child: Column(children: [
        _summaryRow(Icons.timelapse_rounded, 'Total Durasi', _lemburStr, const Color(0xFF135BEC)),
        const SizedBox(height: 8),
        _summaryRow(Icons.payments_rounded, 'Estimasi Kompensasi', _kompensasiStr, const Color(0xFF2ECC71)),
      ]),
    );
  }
      ...
      child: ElevatedButton(
        ...
        child: _submitting
            ...
            : Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                const Text('Kirim Pengajuan', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold)),
                const SizedBox(width: 8),
  ...
  Widget _cancelBtn() {
    return SizedBox(width: double.infinity,
      child: TextButton(
        ...
        child: Text('Batal', style: TextStyle(fontSize: 14, color: Colors.grey.shade500, fontWeight: FontWeight.w500)),
      ),
    );
  }
}