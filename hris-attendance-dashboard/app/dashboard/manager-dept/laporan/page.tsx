...
export default function DeptReportsPage() {
  ...
  }, [period, departmentId, eventType, approvalStatus, query]);
  ...
  const onExport = () => {
    const filename = `laporan-presensi_${departmentName}_${period.value}.csv`;
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Laporan Aktivitas Departemen</h1>
          <p className="text-gray-600">
            Aktivitas presensi karyawan di departemen <span className="font-semibold">{departmentName}</span>.
          </p>
        </div>
      ...
      <Card className="rounded-2xl">
        <CardContent className="p-5">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <FilterChip label={`Dept: ${departmentName}`} />
            {eventType !== "all" && (
              <FilterChip
                ...
                <tbody className="divide-y divide-gray-100 bg-white">
                  {filtered.map((r) => (
                    <tr key={r.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <Td>
                        <div className="text-sm font-medium text-gray-900">{r.position}</div>
                      </Td>
