...
func (h *AuthHandler) Login(c *gin.Context) {
    ...
    response, err := h.authService.Login(c.Request.Context(), req) // ← Remove & (pass value, not pointer)
    if err != nil {
...
func (h *AuthHandler) Register(c *gin.Context) {
    ...
    user, err := h.authService.Register(c.Request.Context(), req) // ← Remove & (pass value, not pointer)
    if err != nil {
        c.JSON(http.StatusBadRequest, models.ErrorResponse("Bad Request", err.Error()))
        return
    }
...
func (h *AuthHandler) Logout(c *gin.Context) {
    ...
    // In MongoDB, we don't need to invalidate token server-side
    // Token will expire naturally or client should remove it
    // This is just a placeholder for logout logic
